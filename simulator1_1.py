...
operative_statblocks = pd.read_csv('op_data.csv', index_col = False).fillna(0)
...
weapon_statblocks = pd.read_csv('weap_data.csv', index_col = False).fillna(0)
...
# factions_fireteams_operatives = pd.read_csv('factions_fireteams_operatives.csv', index_col = False)
# factions_fireteams = pd.read_csv('factions_fireteams.csv', index_col = False)
# fireteams_operatives = pd.read_csv('fireteams_operatives.csv', index_col = False)
...
def hit_calculator(A, WBS, crit, Relentless, Ceaseless, Balanced, Rending, Hot):
    ...
    if Ceaseless == True:
        ...
        hits[1] = reroll[1]
        hits[2] = hits[2] + reroll[2]
    if Balanced == True and hits[0] + hits[2] < A:
        reroll = d6(1, WBS, crit)
        hits[0] = hits[0] + reroll[0]
        hits[1] = max(0, hits[1]-1) + reroll[1]
        hits[2] = hits[2] + reroll[2]
    if Rending == True and hits[0] >= 1 and hits[2] >= 1:
        hits[0] = hits[0] - 1
        hits[2] = hits[2] + 1
...
def parry_calculator(attacker_hits, defender_hits, attacker_defensive, attacker_shield, attacker_normal_damage, attacker_crit_damage, attacker_stun, defender_stunned, defender_HP, defender_brutal):
    ...
    if attacker_defensive == 0 or (attacker_stun == True and defender_stunned == 0 and defender_hits[0] > 0): # If attacker is aggressive, it will always choose to deal damage, also if it can remove opponents attack by stunning
        strike = 1
    else:
...
faction_options = [{'label': i, 'value': i} for i in sorted(set(operative_statblocks['Faction']))]
faction_options = [{'label': 'All','value': 'All'}] + faction_options

fireteam_options = [{'label': i, 'value': i} for i in sorted(set(operative_statblocks['Fire team']))]
fireteam_options = [{'label': 'All','value': 'All'}] + fireteam_options

operative_options = [{'label': i, 'value': j} for i,j in sorted(set(zip(operative_statblocks['Operative type'], operative_statblocks.index)))]
# operative_options = [{'label': '','value': ''}] + operative_options

weapon_options=[]
#weapon_options = [{'label': i, 'value': j} for i,j in sorted(set(zip(weapon_statblocks.loc[:,'Weapon name'], weapon_statblocks.index)))]
...
app.layout = html.Div([
    html.H1(style={'padding': '10px'}, children = 'Kill Team 2.0 simulator'),
...
def update_graph(n_clicks, FF, operative_first, turns_max, random_order, num_simulations, operative_row, operative_weapon_row, operative_cover, operative_defensive, operative_shield, target_row, target_weapon_row, target_cover, target_defensive, target_shield):
    ...
    if operative_row > -1 and operative_weapon_row > -1 and target_row > -1 and target_weapon_row > -1:
        print(operative_statblocks.loc[operative_row,'Operative type'], weapon_statblocks.loc[operative_weapon_row,'Weapon name'], operative_statblocks.loc[target_row,'Operative type'], weapon_statblocks.loc[target_weapon_row,'Weapon name'])
...
def filter_operative_on_faction(o_faction):
    if o_faction == 'All':
        ...
        o_type = [{'label': i, 'value': j} for i,j in sorted(set(zip(operative_statblocks.loc[operative_statblocks['Faction'] == o_faction,'Operative type'],operative_statblocks.index[operative_statblocks['Faction'] == o_faction])))]
...
def filter_target_on_faction(t_faction):
    if t_faction == 'All':
        ...
        t_type = [{'label': i, 'value': j} for i,j in sorted(set(zip(operative_statblocks.loc[operative_statblocks['Faction'] == t_faction,'Operative type'],operative_statblocks.index[operative_statblocks['Faction'] == t_faction])))]
...
def filter_operative_on_fireteam(o_fireteam, o_faction):
    if o_fireteam == 'All':
        ...
        o_type = [{'label': i, 'value': j} for i,j in sorted(set(zip(operative_statblocks.loc[operative_statblocks['Fire team'] == o_fireteam,'Operative type'],operative_statblocks.index[operative_statblocks['Fire team'] == o_fireteam])))]
    return o_type
@app.callback(
Output('t_type', 'options'),
...
def filter_target_on_fireteam(t_fireteam, t_faction):
    if t_fireteam == 'All':
        if t_faction == 'All':
         ...
         t_type = [{'label': i, 'value': j} for i,j in sorted(set(zip(operative_statblocks.loc[operative_statblocks['Fire team'] == t_fireteam,'Operative type'],operative_statblocks.index[operative_statblocks['Fire team'] == t_fireteam])))]
    return t_type
...
Input('o_type', 'value'),
Input('FF', 'value'),)
def filter_operative_weapon(o_type, FF):
    # if FF == 1:
    #     combat_type = 'FF'
    # else:
    #     combat_type = 'HtH'
    # print(o_type)
    # print(weapon_statblocks['Operative index'])
    o_weapon = [{'label': i, 'value': j} for i,j in sorted(set(zip(weapon_statblocks.loc[(weapon_statblocks['Operative index'] == o_type) & (weapon_statblocks['FF'] == FF),'Weapon name'],weapon_statblocks.index[(weapon_statblocks['Operative index'] == o_type) & (weapon_statblocks['FF'] == FF)])))]
    return o_weapon
@app.callback(
Output('t_weapon', 'options'),
Input('t_type', 'value'),
Input('FF', 'value'),)
def filter_target_weapon(t_type, FF):
    # if FF == 1:
    #     combat_type = 'FF'
    # else:
    #     combat_type = 'HtH'
    t_weapon = [{'label': i, 'value': j} for i,j in sorted(set(zip(weapon_statblocks.loc[(weapon_statblocks['Operative index'] == t_type) & (weapon_statblocks['FF'] == FF),'Weapon name'],weapon_statblocks.index[(weapon_statblocks['Operative index'] == t_type) & (weapon_statblocks['FF'] == FF)])))]
    return t_weapon
